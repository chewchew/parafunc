...
parMap4 d f ls = runEval $ go d f ls
  where
    go _ _ [] = return []
    go d f ls = do
...
benchmarkIt = do
  ...
  defaultMain 
         ...
         (bench "jackknife rpar/rseq/rseq with chunks of 200" 
            (nf (jackknife (parMap4 200 mean)) rs)),
...
run = do
  ...
  let ls = (jackknife (parMap3 mean) (take 6000 (randoms (mkStdGen 211570155))::[Float]))
